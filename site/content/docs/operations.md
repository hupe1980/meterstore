...
| `meterstore.subjects_erased` | **Compliance rather than health**, and the only irreversible operation this crate performs — so it is the one counter here whose *rise* is worth a look, and the one whose flat zero is too. Split by a `trigger` attribute, because the two readings are opposite: `trigger="retention"` is the § 60 Abs. 6 sweep, where a flat zero over a year is a job that is not running and a spike is a cohort reaching the ceiling together; `trigger="request"` is an Article 17 erasure, where a flat zero is ordinary. Summed, a deployment whose sweep had silently stopped but which handled the occasional request would look like one whose sweep was working. No `table` attribute — the registry is deployment-wide. |
